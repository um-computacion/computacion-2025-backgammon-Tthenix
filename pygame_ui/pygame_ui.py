...
class BackgammonBoard:
    ...
    def draw_triangular_point(self, surface: pygame.Surface, x: int, y: int,
                            width: int, height: int, color: Tuple[int, int, int],
                            pointing_up: bool = True) -> None:
        """
    ...
    def draw_center_gap(self, surface: pygame.Surface) -> None:
        ...
        gap_rect = pygame.Rect(gap_x, gap_y, self.center_gap_width, self.play_area_height)
        ...
        shadow_rect = pygame.Rect(gap_x + self.center_gap_width - shadow_width, gap_y,
                                shadow_width, self.play_area_height)
        pygame.draw.rect(surface, self.colors['center_gap_shadow'], shadow_rect)
        ...
        for i in range(0, self.center_gap_width, 6):
            ...
            pygame.draw.line(surface, line_color,
                           (gap_x + i, gap_y),
                           (gap_x + i, gap_y + self.play_area_height), 1)
        ...
        for y in range(gap_y, gap_y + self.play_area_height, 12):
            ...
            pygame.draw.line(surface, line_color,
                           (gap_x + 2, y),
                           (gap_x + self.center_gap_width - 2, y), 1)
    ...
    def draw_bear_off_area(self, surface: pygame.Surface) -> None:
        ...
        Args:
            surface: Surface to draw on
        """
        bear_off_rect = pygame.Rect(self.bear_off_x, self.bear_off_y, 
                                   self.bear_off_width, self.board_height)
        ...
        for i in range(0, self.bear_off_width, 8):
            ...
            pygame.draw.line(surface, line_color,
                           ...
                           (self.bear_off_x + i, self.bear_off_y + self.board_height), 1)
        ...
        pygame.draw.line(surface, self.colors['bear_off_border'],
                        (self.bear_off_x, middle_y),
                        (self.bear_off_x + self.bear_off_width, middle_y), 2)
    ...
    def _draw_bear_off_labels(self, surface: pygame.Surface) -> None:
        ...
        try:
            ...
            white_rect.center = (self.bear_off_x + self.bear_off_width // 2,
            ...
            black_rect.center = (self.bear_off_x + self.bear_off_width // 2,
            ...
            pygame.draw.circle(surface, self.colors['white'],
                             (self.bear_off_x + self.bear_off_width // 2,
                              self.bear_off_y + self.board_height // 4), 8)
            pygame.draw.circle(surface, self.colors['black'],
                             (self.bear_off_x + self.bear_off_width // 2,
                              self.bear_off_y + self.board_height // 4), 8, 2)
            
            # Black area indicator (filled circle)
            pygame.draw.circle(surface, self.colors['black'],
                             (self.bear_off_x + self.bear_off_width // 2,
                              self.bear_off_y + 3 * self.board_height // 4), 8)
    ...
    def draw_board(self) -> None:
        ...
        wood_surface = self.create_wood_texture_surface(self.board_width, self.board_height)
        self.screen.blit(wood_surface, (self.board_x, self.board_y))

        # Draw dark border around the entire board
        border_rect = pygame.Rect(self.board_x, self.board_y,
                                self.board_width, self.board_height)
        pygame.draw.rect(self.screen, self.colors['border_dark'], border_rect, self.border_width)
    ...
    def draw_points(self) -> None:
        ...
        for i in range(6):
            ...
            color = self.colors['point_light'] if i % 2 == 0 else self.colors['point_dark']
            self.draw_triangular_point(
                self.screen, point_x, self.play_area_y,
                self.point_width, self.point_height, color, pointing_up=False
            )

            # Bottom points (12-7) - pointing up
            color = self.colors['point_dark'] if i % 2 == 0 else self.colors['point_light']
            self.draw_triangular_point(
                self.screen, point_x,
                self.play_area_y + self.play_area_height - self.point_height,
                self.point_width, self.point_height, color, pointing_up=True
            )
